...
public class CountBlock : MonoBehaviour
{
    ...
    void OnTriggerExit(Collider other)
    {
        ...
        //자신(블록)이 해당하는 블록 배열의 인덱스를 가져옴(이름에서 가져옴)
    ...
    void OnTriggerExit(Collider other)
    {
        //충돌한 블록의 이름을 가져옴
        blockName = other.gameObject.name;
        //블록이 해당하는 블록 배열의 인덱스를 가져옴(블록의 이름에서 가져옴)
        splitText = blockName.Split(separatorChar);   //'('와 ')'를 기준으로 string 분할
        string test = splitText[1];
        index = int.Parse(test); //string형 index를 int로 변환
